...
def fetch_(version, book, chapter, verses=None):
  if verses:
    ...
    for i in range(len(verses)):
      ref.append('references[%s]=%s' % (i, '%s.%s.%s' % (book, chapter, verses[i])))

    url = '{base}/verses/3.1?id={version}&{references}&format=text'.format(base=BIBLE_COM_BASE_URL,
                                                                           version=version,
                                                                           references='&'.join(ref))
  else:
    url = '{base}/chapter/3.1?id={version}&reference={book}.{chapter}'.format(base=BIBLE_COM_BASE_URL,
                                                                              version=version,
                                                                              book=book,
                                                                              chapter=chapter)
  ...
  if not verses:
    ...
    for verse in response['verses']:
      ...
      passage_verses.append({
        ...
        'content': u''.join((verse['content'])).encode('utf-8').strip()
      })
